  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
      {models.map((vehicle, index) => (
        <div key={index} className="hover:shadow-lg hover:shadow-hyundai transition-all duration-700 text-center">
          <div onClick={() => handleImageClick(vehicle)}>
            <img src={vehicle.urlImgView || 'https://placehold.co/600x400'} alt="Description" width={500} height={300} />
            <h3 className="uppercase text-hyundai font-semibold">{vehicle.model}</h3>
          </div>
        </div>
      ))}
    </div>
