...
export default function Services() {
    ...
    useEffect(() => {
      getAPI('dealer') 
        ...
        .catch(error => {console.error('Failed to load dealers:', error)
    ...
    const handleSubmit = (event) => {
      ...
      if (whatsappNumber) {
        const message = `
          ...
          Nomor Kendaraan: ${formData.get('vehicleNumber')}
    ...
    <div className="max-w-xl mx-auto p-5 shadow-lg shadow-hyundai rounded-lg">
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <input type="text" id="vehicleNumber" name="vehicleNumber" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-hyundai focus:border-hyundai" placeholder="B 1234 XYZ" required />
        </div>

        <div>
          <label htmlFor="vehicleType" className="block font-medium text-gray-700">Jenis Kendaraan</label>
          <select id="vehicleType" name="vehicleType" className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-hyundai focus:border-hyundai rounded-md" required>
        ...
        <div>
          ...
          <select id="dealer" name="dealer" className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-hyundai focus:border-hyundai rounded-md" required onChange={(e) => setSelectedDealer(e.target.value)}>
            {dealers.map(dealer => (
